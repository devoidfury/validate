...
;(function(window, $, undefined){
    ...
    $.validate = function(settings, $form){
        ...
        function extractParams(funcName){
            ...
            var params = funcName.slice(funcName.indexOf('[') + 1, -1);

            return params.indexOf(',') !== -1 ? params.split(',') : [params];
        }
         ...
         * @returns {boolean}
         */
        function shouldAutoDetect($field) {
         ...
         * Validate the form
         *
         * @param  {Object} $f jQuery selection object
         * @return {boolean}
         */
        plugin.validate = function($f) {
            var $toCheck = (typeof $f === 'undefined') ? plugin.$form : $f,
                $fields,
                error = false,
                failedFields = $();
            ...
            } else if (o.overallSuccess.call(plugin, $toCheck) === false) {
                error = true;
            }
         ...
         * @returns {Array|boolean}        Returns true if valid, or an array of rules failed if not
         */
        plugin.checkValue = function($field, rules){

            if (!rules) return true;

            rules = (typeof rules == 'string') ? processRules(rules) : rules;
        ...
        plugin.fieldsToCheck = function($f){
            var $toCheck = ($f === undefined) ? plugin.$form : $f,
                $fields;

            // We need to do this every time in case we've added new fields
        ...
        plugin.checkRegExp = function($field, regExp){
            return !!$field.val().match(o.regExp[regExp]);
        };
        ...
        /**
         * Checks if the value looks like a phone number
         *
         * @param  {Object} $field
         * @returns {boolean}
         */
        plugin.checkPhone = function($field){
            if($field.val() === '') return true;

            return !!$field.val().split(/\D/).join('').match(/^\d{10}/);
        };

        // handy dandy teleportation device
        plugin.scrollToError = function($field) {
            $('html, body').animate({
